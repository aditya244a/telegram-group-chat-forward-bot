...
class TelegramForwarder:
    def __init__(self, api_id, api_hash, phone_number, source_chat_ids, destination_channel_id, topic_id, keywords):
        self.api_id = api_id
    ...
    async def _forward_message(self, message, source_chat_id):
        try:
            ...
            if message.text:
                ...
                await self.client.send_message(self.destination_channel_id, combined_message, reply_to=self.topic_id)
            elif message.media:
                caption = getattr(message, 'caption', '')
                full_caption = f"{header_message}\n\n{caption}" if caption else header_message
                await self.client.send_file(self.destination_channel_id, message.media, caption=full_caption, reply_to=self.topic_id)
            else:
                await self.client.forward_messages(self.destination_channel_id, message.id, source_chat_id, reply_to=self.topic_id)
